...
def search_wine(dish_flavor):
    result_with_scores = vector_store.similarity_search_with_score(
        dish_flavor,
        k=2
    )
    # 결과를 유사도와 함께 출력
    ...
    for doc, score in result_with_scores:
        reveiw_text = f"유사도: {score:.4f}\n내용: {doc.page_content}"
        wine_reviews.append(reveiw_text)
    return {
...
def recommand_wine(inputs):
    prompt = ChatPromptTemplate.from_messages([
        ("system", """ """),
        ("human", """
            와인 페어링 추천에 아래의 요리의 풍미와 와인 리뷰를 참고해 한글로 답변해주세요.
            추천된 와인이 두 개여야 하고, 이를 검증한 다음에 두 개의 추천된 와인 중에 가장 어을리는 와인을 추천해주세요.
            위의 두 과정을 다시 한 번 검토해서 의도대로 답변해 주세요.
